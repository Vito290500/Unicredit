...
function setupBonificoFlow(accessToken) {
  ...
  pinConfirmBtn.addEventListener('click', function() {
    ...
    .then(({ status, data }) => {
      ...
      if (status === 201) {
        hidePinModal();
        showSuccessModal();
        // Recupero dati mittente dal form
        ...
        // Salvo i dati della transazione per l'anteprima PDF
        lastTransactionData = {
  ...
  // Aggiunto gestione salvataggio contatto in rubrica
  if (saveContactBtn) {
...
// Funzione completa per generare e mostrare l'anteprima PDF della ricevuta
function generateAndPreviewPDF(data) {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();
  ...
  getBase64FromImageUrl('/static/image/logo.png', function(logoBase64) {
    ...
    doc.text(`IBAN: ${data.mittente?.iban || ''}`, 20, y+=8);
    doc.text(`Città: ${data.mittente?.citta || ''}`, 20, y+=8);
    // Aggiungo eventuali dati mittente mancanti qui, esempio:
...
/* Variabile globale per salvare i dati della transazione */
window.lastTransactionData = null;

/* Modifico la gestione chiusura modale successo per mostrare l'anteprima PDF senza alert */
const viewReceiptBtn = document.getElementById('view-receipt-btn');
if (viewReceiptBtn) {
  viewReceiptBtn.addEventListener('click', function() {
    if (lastTransactionData) {
    ...
    // Se non ci sono dati, non fare nulla (rimuovo alert)
  });
}
